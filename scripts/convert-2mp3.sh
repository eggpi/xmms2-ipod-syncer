...
#
# For a list of allowed options please consult convert-common.sh
#
...
#   7 unknown option
#
# This script is adapted from gtkpod, which is available under the
...
if [ $filetype = "wav" ]; then
    "$encoder" $ENCODER_OPTS --add-id3v2 --tt "$title" --ta "$artist" --tl "$album" --ty "$year" --tc "$comment" --tn "$track" --tg "$usegenre" "$infile" "$outfile" >/dev/null 2>&1
else
    "$decoder" $options "$infile" | "$encoder" $ENCODER_OPTS --add-id3v2 --tt "$title" --ta "$artist" --tl "$album" --ty "$year" --tc "$comment" --tn "$track" --tg "$usegenre" - "$outfile" >/dev/null 2>&1
fi
# Check result
if [ "x$?" != "x0" ]; then
    exit 6
...
# Seems to be ok: display filename
echo $outfile
exit 0
